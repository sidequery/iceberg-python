...
def test_read_spark_written_puffin_dv(spark: SparkSession, session_catalog: RestCatalog) -> None:
    ...
    deletion_vectors = deletion_vectors_from_puffin_file(puffin)
    assert len(deletion_vectors) == 1, "Expected one data file's deletions"

    for deletion_vector in deletion_vectors:
        positions = deletion_vector.to_vector().to_pylist()
